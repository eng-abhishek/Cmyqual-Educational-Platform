...
.modal-list.modal-list-form ul li input[type=url], .modal-list.modal-list-form ul li input[type=number]
{
...
#standard
{
  margin: 5px 0 0px 22px!important;
}
.recent-learn.cpd-table table tr td a
{
    display: block;
    text-align: center;
...
h4.overview-drop , .new-figma-recent h4
{
  padding-top:20px;
}
      .goals-border
...
body.leftmenu #wrapper.toggled #page-content-wrapper .team-member-name.cpd-list {
    margin-top: -40px;
}
.team-member-name.cpd-list
{
  margin-top: -50px;
}
.recent-learn
...
.col-lg-12.new-figma-recent
{
  margin-top:60px;
}
#title , .expDate , #provider ,#standard{
...
@foreach($cpdLogInfo as $cpdLogInfoData)
      <tr>
        <td style="width: 100px;"><b>{{date('d-m-Y',strtotime($cpdLogInfoData->date))}}</b></td>
        ...
        <form method="post" action="{{url('add-cpd-log')}}" id="cpd-log"> 
         ...
         <ul>
          ...
          <input type="number" placeholder="5 Hours" name="hours" id="hours" maxlength="5" min="0" required=""><!-- <i class="fa fa-pencil-square-o a-pencil pencil1" aria-hidden="true"></i> -->
           ...
           <textarea id="notes" name="notes" placeholder="Summary" maxlength="150" minlength="5"></textarea>
           </li>
